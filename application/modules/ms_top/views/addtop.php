...
<div class="box box-primary">
    <div class="box-body">
		<form id="data-form" method="post">
			...
			<div class="row">
		    <center><label for="customer" ><h3>Term Of Payment</h3></label></center>
		            <div class="col-sm-12">
					    <div class="col-sm-12">
                    ...
                    <a class="btn btn-danger btn-sm" href="<?= base_url('/ms_top/') ?>"  title="Edit">Kembali</a>
                    </center>
		</form>		  
	</div>
</div>	
			...
			swal({
				...
				function(isConfirm) {
				  if (isConfirm) {
						...
						var baseurl=siteurl+'ms_top/SaveNewTop';
						$.ajax({
							url			: baseurl,
							type		: "POST",
							data		: formData,
...
function DelItem(id){
    ...
    function GetSubProduk(no){ 
      	var jumlah	=$('#list_spk').find('tr').length;
        var idTop 	= $('#used_top_'+no).val();
		$.ajax({
            type:"GET",
            url:siteurl+'ms_top/GetSubProduk',
            data:"jumlah="+jumlah+"&idtop="+idTop,
            success:function(html){
               $("#list_spk").append(html);
			   $('.select').select2({
				   width:'100%'
