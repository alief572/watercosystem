...
class Masterrate_ferrous extends Admin_Controller
{
	//Permission
	...
	public function index()
	{
		...
		$this->template->title('Master Rate');
	...
	public function deleteRate()
	{
		...
		$this->db->trans_begin();
		$this->db->delete('ms_rate_ferreous', array('id_rate' => $id));
	...
	public function SaveEditRate()
	{
		...
		$data = [
			...
			'id_type_rate'	=> $post['id_type_rate'],
			'presentase_rate'	=> $post['presentase_rate'],
			'modified_on'			=> date('Y-m-d H:i:s'),
			'modified_by'			=> $this->auth->user_id()
		];

		$this->db->where('id_rate', $id)->update("ms_rate_ferreous", $data);
	...
	public function saveNewRate()
	{
		...
		if (empty($_POST['data1'])) {
			...
			foreach ($_POST['data1'] as $d1) {
				...
				//Add Data
				$this->db->insert('ms_rate_ferreous', $data1);
			}
		}
