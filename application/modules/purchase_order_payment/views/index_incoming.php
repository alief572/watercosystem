	...
	<div class="box-body">
		...
		<?php if(empty($results)){
			...
			$kurs       = 0;
				...
				// $noincoming = $record->id_data;		
				// $incoming = $this->db->query("select id_dt_po FROM dt_incoming WHERE id_data='$noincoming'")->result();
				// $separator =',';
				// $allinv = array();
		...
		<tr>
			...
			<?php if($ENABLE_MANAGE) : 
			if($record->status == '2'){?>
				<a class="btn btn-warning btn-sm" href="<?= base_url('/purchase_order_payment/request_bayar/'.$record->no_po.'/'.$record->id_data) ?>" title="Request Pembayaran" ><i class="fa fa-list"></i></a>
