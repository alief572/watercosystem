...
class Incoming extends Admin_Controller
{
	...
	public function index_jurnal()
	{
		...
		$session = $this->session->userdata('app_session');
		$this->template->page_icon('fa fa-users');
		$data = $this->db->query("SELECT a.*, b.name_suplier as name_suplier FROM tr_incoming as a INNER JOIN master_supplier as b on a.id_suplier=b.id_suplier WHERE a.status_jurnal='OPN' ORDER BY a.created_date DESC")->result();
		$this->template->set('results', $data);
		$this->template->title('Incoming');
	...
	public function SaveNew()
	{
		...
		foreach ($Detail as $val => $valx) {
			...
			$ArrHeader	 = [
				...
				'no_po'				=> $nomor_po,
				'total_ppn'		    => $totalppn * $nilai_kurs,
				'total_barang'		=> $totalbarang * $nilai_kurs,
			];
