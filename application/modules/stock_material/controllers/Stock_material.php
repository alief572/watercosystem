...
class Stock_material extends Admin_Controller
{
	//Permission
	...
	public function __construct()
	{
		...
		$this->load->model(array(
			...
			'stock_material/Serverside_model',
	...
	{
		$this->Serverside_model->getDataJSON();
	}

	public function data_side_material_grw()
	{
		$this->Serverside_model->getDataJSON_GRW();
	}

	public function data_side_material_booking()
	{
		$this->Serverside_model->getDataJSON_booking();
	}
	...
	public function GudangRawMaterial()
	{
		...
		$this->template->set('results', $data);
		$this->template->title('Stock Gudang I');
		$this->template->render('gudang_raw_material');
	...
	public function GudangFinishGood()
	{
		...
		$this->template->set('results', $data);
		$this->template->title('Stock Gudang II');
		$this->template->render('gudang_fg');
	}

	public function perbooking()
	{
		$id_gudang = '3';
		$this->auth->restrict($this->viewPermission);
		$session = $this->session->userdata('app_session');
		...
		$this->template->set('results', $data);
		$this->template->title('Stock Booking');
		$this->template->render('perbooking');
	}
	...
	public function GudangScrap()
	{
		...
		$this->template->set('results', $data);
		$this->template->title('Stock Material Scrap');
		$this->template->render('gudang_scrap');
	}
	public function GudangSubcont()
	{
		...
		$this->template->set('results', $data);
		$this->template->title('Stock Material SubCont');
		$this->template->render('gudang_subcont');
	}
	...
	public function EditStock($id)
	{
		...
		$this->template->set('results', $data);
		$this->template->title('Add Stock');
		$this->template->render('EditStock');
	}
	...
	public function PrintHeader($kategori = null)
	{
		ob_clean();
		ob_start();
		...
		$where_kategori = "";
		if (!empty($kategori)) {
			$where_kategori = " AND c.id_category2 = '" . $kategori . "' ";
		}
		$data['header']  = $this->db->query("	SELECT 
													a.* 
												FROM 
													stock_material a
													LEFT JOIN ms_inventory_category3 b ON a.id_category3=b.id_category3
													LEFT JOIN ms_inventory_category2 c ON b.id_category2=c.id_category2
												WHERE 1=1
													" . $where_kategori . "
											")->result();
	...
	public function saveNewStock()
	{
		...
		$data = [
			'id_stock'				=> $code,
			'id_category3'			=> $post['id_inventory'],
			'nama_material'			=> $post['nm_inventory'],
			'width'					=> $post['width'],
			'lotno'					=> $post['lotno'],
			'qty'					=> $post['qty'],
			...
			'weight'				=> $post['weight'],
			'totalweight'			=> $post['qty'] * $post['weight'],
			'aktif'					=> 'Y',
			'id_gudang'				=> $post['status'],
	...
	public function GudangNcr()
	{
		...
		$this->template->set('results', $data);
		$this->template->title('Stock Material NCR');
		$this->template->render('gudang_ncr');
	}
	...
	public function view_kartu_stok_edit()
	{
		$this->auth->restrict($this->viewPermission);
		$session = $this->session->userdata('app_session');
		$this->template->page_icon('fa fa-users');
		...
		$this->template->title('Kartu Stok');
		$this->template->render('index_kartu_stok_edit');
	}

	public function editKartustok($id)
	{
		$this->auth->restrict($this->viewPermission);
		$session = $this->session->userdata('app_session');
		$this->template->page_icon('fa fa-pencil');
		...
		$all   = $this->db->query("SELECT a.*, b.nama FROM kartu_stok a
		INNER JOIN ms_inventory_category3 b on a.id_category3 = b.id_category3
		WHERE a.id_kartu_stok='$id'")->result();
		...
		$this->template->set('results', $data);
		$this->template->title('Edit Kartu Stok');
		//        $this->template->render('edit_inventory');
		$this->template->render('form_edit_kartustok');
	}



	public function update_kartustok()
	{

		$dtpr = $this->db->query("SELECT a.* FROM kartu_stok as a")->result();
		foreach ($dtpr as $dt) {
			$transaksi = $dt->transaksi;
			$no_transaksi = $dt->no_transaksi;

			if ($transaksi == 'sales order') {
				$so = $this->db->query("SELECT a.* FROM tr_sales_order as a WHERE no_so='$no_transaksi'")->row();
				$no = $so->no_surat;
				$this->db->query("UPDATE kartu_stok SET no_surat='$no' WHERE no_transaksi='$no_transaksi'");
			} else if ($transaksi == 'delivery order') {
				$so = $this->db->query("SELECT a.* FROM tr_delivery_order as a WHERE no_do='$no_transaksi'")->row();
				$no = $so->no_surat;
				$this->db->query("UPDATE kartu_stok SET no_surat='$no' WHERE no_transaksi='$no_transaksi'");
			}
		};
	...
	public function GudangRawMaterialBackdate()
	{
		$id_gudang = '1';
		$this->auth->restrict($this->viewPermission);
		$session = $this->session->userdata('app_session');
		...
		$this->template->set('results', $data);
		$this->template->title('Stock Gudang I Back Date');
		$this->template->render('gudang_raw_material_backdate');
		// $this->template->render('gudang');
	}

	public function data_side_material_grw2()
	{
		$this->Serverside_model->getDataJSON_GRW2();
	}

	public function GudangRawMaterialHarga()
	{
		$id_gudang = '1';
		$this->auth->restrict($this->viewPermission);
		$session = $this->session->userdata('app_session');
	...
	public function data_side_material_grw_harga()
	{
		$this->Serverside_model->getDataJSON_GRW_harga();
	}


	public function saveEditKartustok()
	{
		$this->auth->restrict($this->addPermission);
		$session = $this->session->userdata('app_session');
		...
		$header1 =  array(
			'qty'		       		=> $post[qty],
			'qty_book'		    	=> $post[qty_book],
			'qty_free'		    	=> $post[qty_free],
			'qty_akhir'		        => $post[qty_akhir],
			'qty_book_akhir'		=> $post[qty_book_akhir],
			'qty_free_akhir'	    => $post[qty_free_akhir],
			'modified_on'			=> date('Y-m-d H:i:s'),
			'modified_by'			=> $this->auth->user_id(),
		);
		$this->db->where('id_kartu_stok', $id)->update("kartu_stok", $header1);
