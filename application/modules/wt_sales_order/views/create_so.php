...
<div class="box box-primary">
    <div class="box-body">
		<form id="data-form" method="post">
			...
			<div class="row">
		<center><label for="customer" ><h3>Sales Order</h3></label></center>
		<div class="col-sm-12">
		    <div class="col-sm-6">
		        <div class="form-group row">
			        ...
			        <div class="col-md-8">
				        <input type="text" class="form-control" id="no_surat" required name="no_surat" readonly placeholder="No. Sales Order">
			        </div>
		        </div>
		    </div>
		    <div class="col-sm-6">
		...
		<div class="col-sm-12">
		    <div class="col-sm-6">
		        <div class="form-group row">
			        <div class="col-md-4">
				        <label for="customer">Upload Po</label>
			        </div>
			        <div class="col-md-8">
				        <input type="file" class="form-control" id="upload_po" required name="upload_po" readonly placeholder="Upload Po">
			        </div>
		        </div>
		    </div>
		    <div class="col-sm-6">
		        <div class="form-group row">
			        <div class="col-md-4">
				        <label for="customer">Upload Confirm SO</label>
			        </div>
			        <div class="col-md-8">
					<input type="file" class="form-control" id="upload_so" required name="upload_so" readonly placeholder="Upload Confirm So">
			        </div>
		        </div>
		    </div>
		</div>
		...
		<div class="col-md-12">
		    <div class='col-sm-6'>
		        <div class='form-group row'>
			        ...
			        <div class='col-md-8'>
				        <input type='text' class='form-control' id='reff' required name='reff'  >
			        </div>
		        </div>
		    </div>
		    <div class='col-sm-6'>
			    <div class='form-group row'>
				    <div class='col-md-4'>
					    <label for='id_category_supplier'></label>
				    </div>
				    <div class='col-md-8' id="pic_slot" >
        ...
        <?php } ?>
								...
								<table class='table table-bordered table-striped'>
									...
									foreach ($results['detail'] as $dt_spk){$loop++; 
                                        ...
                                        echo	"</select>
											...
											<td id='tgl_delivery_$loop'><input type='date' class='form-control input-sm' id='used_tgl_delivery_$loop' required name='dt[$loop][tgl_delivery]'></td>
                                         	<td id='qty_so_$loop'><input type='text' class='form-control input-sm qty_so' id='used_qty_so_$loop' required name='dt[$loop][qty_so]' onblur='HitungTotal($loop)'></td>
					...
					-->
					<a class="btn btn-danger btn-sm" href="<?= base_url('/wt_penawaran/index') ?>"  title="Edit">Kembali</a>
					<button type="submit" class="btn btn-success btn-sm" name="save" id="simpan-com2"><i class="fa fa-save"></i>Deal SO</button>
                    </center>
		</form>		  
	</div>
</div>	
...
<script type="text/javascript">
	...
	$('#simpan-com').click(function(e){
			...
			swal({
				...
				function(isConfirm) {
				  if (isConfirm) {
						...
						var baseurl=siteurl+'wt_sales_order/SaveNewSalesOrder';
						$.ajax({
							url			: baseurl,
							type		: "POST",
							data		: formData,
							...
							success		: function(data){								
								if(data.status == 1){											
									...
									window.location.href = base_url + 'wt_sales_order';
								}else{
		...
		$('#simpan-com2').click(function(e){
			...
			var image	= $('#image').val();
			var idtype	= $('#inventory_1').val();
			var uploadpo	= $('#upload_po').val();
			var uploaddo	= $('#upload_so').val();
			var tanggal  =$('#tanggal').val();
			var qtyso	= $('.qty_so').val();
			var reff	= $('#reff').val();

			$(".select").removeAttr("disabled");
			
			var data, xhr;
			if(uploadpo =='' && uploaddo==''){
					swal("Warning", "Dokumen upload Tidak Boleh Kosong :)", "error");
					return false;
			}
			if(qtyso =='' || qtyso ==0){
					swal("Warning", "QTY SO Tidak Boleh Kosong :)", "error");
					return false;
			}
			if(reff ==''){
					swal("Warning", "Reff Tidak Boleh Kosong :)", "error");
					return false;
			}
			if(tanggal ==''){
					swal("Warning", "Tanggal SO Tidak Boleh Kosong :)", "error");
					return false;
			}else{
			...
			swal({
				...
				function(isConfirm) {
				  if (isConfirm) {
						var formData 	=new FormData($('#data-form')[0]);
						var baseurl=siteurl+'wt_sales_order/SaveDealNewSalesOrder';
						$.ajax({
							url			: baseurl,
							type		: "POST",
							data		: formData,
...
function DelItem(id){
	...
	$(document).on('blur', '#upload_so', function(){
		var po = $(this).data('upload_so');
		if(po !='')
		{
			$('#simpan-com2').show();
		}
