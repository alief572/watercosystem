...
class Penerimaandn extends Admin_Controller
{
	...
	public function __construct()
	{
		...
		$this->load->model('Penerimaandn/penerimaan_model');
		$this->load->model('Jurnal_nomor/Acc_model');
		$this->load->model('Jurnal_nomor/Jurnal_model');
		$this->template->title('Penerimaan Debit Note');
	...
	{
		$noinvoice = $this->db->query("SELECT no_invoice FROM tr_debit_note_payment_detail WHERE kd_pembayaran = '$kd_bayar' ")->result();
		$buktipotong = $this->db->query("SELECT * FROM tr_debit_note_bukti_potong WHERE kd_pembayaran = '$kd_bayar' ")->result();
		$data = array(
			'kodebayar' => $kd_bayar,
			'noinvoice' => $noinvoice,
	...
	public function save_buktipotong()
	{
		...
		$this->db->insert('tr_debit_note_bukti_potong', $data);
	...
	public function save_penerimaan()
	{
		...
		$this->db->insert('tr_debit_note_payment', $data);
		...
		for ($i = 0; $i < count($this->input->post('kode_produk')); $i++) {
			...
			$this->db->insert('tr_debit_note_payment_detail', $datadetail);
			//Update QTY_AVL
			$invoice = $this->input->post('kode_produk')[$i];
			$jmlbyr  = str_replace(",", "", $this->input->post('jml_bayar')[$i]);
			$Qry_Update	 = "UPDATE tr_debit_note SET total_bayar=total_bayar + $jmlbyr, sisa_tagihan=sisa_tagihan - $jmlbyr WHERE no_pr='$invoice'";
			$this->db->query($Qry_Update);


			$so  = $this->db->query("SELECT * FROM tr_debit_note WHERE no_pr='$invoice'")->row();
			$no_so = $so->no_so;
	...
	function appr_jurnal()
	{
		...
		$data_bayar =  $this->db->query("SELECT * FROM tr_debit_note_payment WHERE kd_pembayaran = '$kd_bayar' ")->row();
		...
		$data_jurnal = $this->db->query("SELECT * FROM tr_debit_note_payment_detail WHERE kd_pembayaran = '$kd_bayar' ")->result();
		...
		$data_jr = $this->db->query("SELECT * FROM tr_debit_note_payment_detail WHERE kd_pembayaran = '$kd_bayar' ")->result();
		...
		$Qry  = "UPDATE tr_debit_note_payment SET status_jurnal='1' WHERE kd_pembayaran='$kd_bayar'";
		$this->db->query($Qry);
	...
	function print_penerimaan_fix()
	{
		...
		$nomordoc   = $data_bayar->nm_customer;
		...
		$data_header = $this->db->query("SELECT * FROM tr_debit_note_header WHERE no_invoice ='$nomordoc'")->row();
		...
		$count = $this->db->query("SELECT COUNT(no_invoice) as total FROM tr_debit_note_detail WHERE no_invoice ='$nomordoc'")->row();
		$count1 = $count->total;
		...
		$header = '
          ...
          <table border="0" width="100%">
            <tr><b>
				 ...
				 <td width="35%" style="font-size:8pt !important;vertical-align:top"><b>' . @$gethd->name_customer . '</b></td>
		 </b> </tr>
		 <tr><b>
	...
	public function TambahInvoice()
	{
		...
		$invoice = $this->db->query("SELECT * FROM tr_debit_note WHERE id_customer ='$customer' AND sisa_tagihan >'0'")->result();
		$data = [
			'detail' => $customer
		];
		$this->template->set('results', $data);
		$this->template->title('List Debit note');
