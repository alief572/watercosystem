<?php
	...
	foreach($ArrJurnal AS $val => $valx){
		...
		$ArrDebit[$Loop]['nm_asset'] 		= $valx['nm_asset'];
		...
		$ArrKredit[$Loop]['nm_asset'] 		= $valx['nm_asset'];
	}
				...
				<?php
					...
					foreach($ArrTemp AS $val => $valx){
						...
						echo "<tr style='background-color:".$colorBack."'>";
							...
							echo "<td align='left'>".$valx['nm_asset']."</td>";
					...
					echo "<tr style='background-color:#36cdfb'>";
						echo "<td colspan='7'><b>TOTAL PENYUSUTAN</b></td>";
					...
					$.ajax({
						...
						success		: function(data){								
							if(data.status == 1){											
								...
								window.location.href = siteurl + 'asset/aset';
							}
							else{ 
								if(data.status == 0){
									swal({
