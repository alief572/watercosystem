...
class Spk_aktual_slitting extends Admin_Controller
{
	//Permission
	...
	public function __construct()
	{
		...
		$this->load->model(array(
			'Spk_aktual_slitting/Inventory_4_model',
	...
	public function index()
	{
		...
		$produksi = $this->db->query("SELECT a.*, b.no_surat as no_spk, b.id_material FROM dt_spk_aktual_detail a 
		INNER JOIN tr_spk_produksi b ON b.id_spkproduksi = a.id_spkproduksi
		WHERE a.slitting='Y' 
		 ")->result();
	...
	public function PrintHeader2()
	{
		...
		$data['header']  = $this->db->query("SELECT a.* FROM tr_spk_aktual_fg as a  where a.id_dt_spkaktualdetail = '" . $id . "' ")->result();

		// $data['detail'] = $this->Inventory_4_model->get_data('dt_spk_produksi','id_spkproduksi',$id);
		$data['detail'] = $this->db->get_where('dt_spk_aktual_detail_fg', array('id_dt_spkaktualdetail' => $id))->result();
		$this->template->title('Data');
	...
	public function EditHeader()
	{
		...
		INNER JOIN tr_spk_aktual b ON b.id_spk_aktual = a.id_spk_aktual  WHERE a.id_spk_aktual = '" . $id2 . "' ")->result();
			...
			$tr_spk = $this->db->query("SELECT a.* ,b.no_surat_produksi, b.tgl_produksi, b.id_material, b.thickness, b.density,b.panjang FROM dt_spk_aktual_detail a 
		INNER JOIN tr_spk_aktual b ON b.id_spk_aktual = a.id_spk_aktual  WHERE a.id = '" . $id . "' ")->result();
	...
	public function TambahLHP()
	{
		...
		$aktif = 'active';
		$deleted = '0';
		$tr_spk = $this->Inventory_4_model->get_data('dt_tr_spk_produksi', 'id_spkproduksi', $id);
	...
	public function ApproveLHPProduksi()
	{
		...
		$spk_produksi 	= $this->db->get_where('dt_spk_aktual_detail', array('id_dt_spk_aktual' => $id))->result();
		...
		foreach ($gudang4 as $key => $value) {
			...
			$scrap[$key]['no_surat'] 		= $value['no_surat'];
		}
		...
		foreach ($gudang6 as $key => $value) {
			...
			$ncr[$key]['no_surat'] 		= $value['no_surat'];
		}
		...
		foreach ($gudang7 as $key => $value) {
			...
			$lost[$key]['no_surat'] 		= $value['no_surat'];
		}
		...
		$this->db->trans_begin();
		$this->db->where('id_dt_spk_aktual', $id)->update("dt_spk_aktual_detail", $data);
	...
	public function SaveEditHeader()
	{
		...
		$this->db->delete('tr_spk_aktual_fg');

		$this->db->insert('tr_spk_aktual_fg', $data);
		...
		$this->db->where('id', $nomor)->update("dt_spk_aktual_detail", $ubahstatus);
		...
		$this->db->where('id_spk_aktual', $id_spk_aktual);
		$this->db->delete('dt_spk_aktual_detail_fg');

		$this->db->insert_batch('dt_spk_aktual_detail_fg', $ArrDetail);
