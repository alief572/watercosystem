...
class Request_payment extends Admin_Controller
{
		...
		if (!empty($status)) {
			foreach ($status as $val) {
				...
				$data =  array(
					...
					'coa_pph' => $this->input->post("pph_coa_" . $val),
					'pph_coa' => $this->input->post("pph_coa_" . $val),
					'persen_pph' => $this->input->post("persenpph_" . $val),
					'nilai_pph' => $this->input->post("totalpph_" . $val),
					'nilai_request' => $this->input->post("jumlah_" . $val),
	...
	*/
	...
	public function approval_payment($type = null, $id = null)
	{
		...
		if (isset($type) && $type == 'periodik') {
			...
			$data_detail	= $this->db->get_where('tr_pengajuan_rutin_detail', ['id' => $data->id])->result();
		}
	...
	*/
	...
	public function view_jurnal($id)
	{
		...
		$this->template->title('Payment Jurnal');
		$this->template->render('view_jurnal');
	}
	public function edit_jurnal($id)
	{
		$data = $this->db->query("select id,tanggal,no_reff,tipe,nomor,no_request,jenis_jurnal,nocust,no_perkiraan,keterangan, sum(debet) as total_debet, sum(kredit) as total_kredit from " . DBERP . ".jurnal where nomor='" . $id . "' GROUP BY no_perkiraan ORDER BY kredit,debet,no_perkiraan")->result();
		$data_coa = $this->All_model->GetCoaCombo();
		$results = $this->Request_payment_model->GetListDataPayment('status=1');
		$this->template->set('data', $data);
		$this->template->set('datacoa', $data_coa);
	...
	public function jurnal_save()
	{
		...
		for ($i = 0; $i < count($id); $i++) {
			...
			$total = ($total + $debet[$i]);
			$this->All_model->DataUpdate(DBERP . '.jurnal', $dataheader, array('nomor' => $nomor));
