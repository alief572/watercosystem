			...
			<tbody>
				<?php if (empty($results)) {
					...
					foreach ($results as $record) {
						$numb++;
						$idbarang = $record->id_category3;
						$kodebarang = $this->db->query("SELECT kode_barang FROM ms_inventory_category3 WHERE id_category3='$idbarang' ")->row();

						$so = $record->no_so;
						$no_so = $this->db->query("select no_surat FROM tr_sales_order WHERE no_so='$so'")->row();
				...
				?>
						<tr>
							<td><?= $numb; ?></td>
							<td><?= $record->id_do_detail ?></td>
							<td><?= $record->no_surat ?></td>
							<td><?= $record->no_so ?></td>
							<td><?= $no_so->no_surat ?></td>
							<td><?= $invc ?></td>
							...
							<td><?= $record->qty_do ?></td>
							<td><?= number_format($record->cost_book, 2) ?></td>
