...
class Spk_produksi extends Admin_Controller
{
	...
	public function __construct()
	{
		...
		$this->load->model(array(
			'Spk_produksi/Inventory_4_model',
	...
	function FindingStock()
	{
		...
		$stok	= $this->db->query("SELECT * FROM stock_material WHERE id_category3 = '$id_material' AND id_gudang='1' AND sisa_spk != 0 ")->result();
		...
		foreach ($stok as $stok) {
			...
			if (!empty($detail_potonganx)) {
				...
				if ($stok->weight > $JUMLAH) {
					echo "<option value='$stok->id_stock'>$stok->lotno|$stok->width|$stok->length|$stok->lotno|$stok->totalweight kgY</option>";
				}
			} else {
				echo "<option value='$stok->id_stock'>$stok->lotno|$stok->width|$stok->length|$stok->totalweight kgx</option>";
			}
		}
		echo "</select>";
	}
	...
	function GetSpk()
	{
		...
		$nosurat = $this->db->query("SELECT a.*, b.no_surat as no_surat FROM dt_spkmarketing as a INNER JOIN tr_spk_marketing as b ON a.id_spkmarketing = b.id_spkmarketing WHERE a.id_material='$id_material' AND a.deal='1' AND b.status_approve = '1' AND close_do='N'")->result();
