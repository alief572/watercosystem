...
class Adjustmentstockmulti extends Admin_Controller
{
	//Permission
	...
	public function __construct()
	{
		...
		$this->load->model(array(
			'Adjustmentstockmulti/Inventory_4_model',
	...
	public function index_display()
	{
		...
		$deleted = '0';
		$stok		 = $this->Inventory_4_model->get_data_display();
		...
		$this->template->title('Adjustment Stock Display');
		$this->template->render('index_display');
	}

	public function index_exdisplay()
	{
		$id_bentuk = $this->uri->segment(3);
		$this->auth->restrict($this->viewPermission);
		$session = $this->session->userdata('app_session');
		$this->template->page_icon('fa fa-users');
		$deleted = '0';
		$stok		 = $this->Inventory_4_model->get_data_exdisplay();
		$history	= $this->db->query("SELECT * FROM adjustment_stock ")->result();
		$data = [
			'stok' => $stok,
			'history' => $history,
		...
		$this->template->set('results', $data);
		$this->template->title('Adjustment Stock Exdisplay');
		$this->template->render('index_exdisplay');
	}
	...
	public function index_retur()
	{
		...
		$this->template->page_icon('fa fa-users');
		$deleted = '0';
		$stok		 = $this->Inventory_4_model->get_data_retur();
		$history	= $this->db->query("SELECT * FROM adjustment_stock ")->result();
		$data = [
			'stok' => $stok,
			'history' => $history,
		...
		$this->template->set('results', $data);
		$this->template->title('Adjustment Stock Retur');
	...
	public function FormMutasi()
	{
		...
		$idgudang  = $post['id_gudang'];
		$idstock   = $post['id_stock'];

		$gudangasal 		= $this->db->query("SELECT * FROM ms_gudang WHERE id_gudang='$idgudang'")->result();
		$gudang 		= $this->db->query("SELECT * FROM ms_gudang")->result();

		if ($idgudang == 1) {
			$stock = $this->db->query("SELECT * FROM stock_material WHERE id_stock='$idstock'")->row();
		} else {
			$stock = $this->db->query("SELECT * FROM stock_material_multigudang WHERE id_stock='$idstock'")->row();
		}
		...
		echo "<div class='form-group row'>
				...
				<div class='col-md-4'>
						...
						<div class='col-md-7'>
						<input type='number' class='form-control' id='mother_qty' readonly required name='mother_qty' placeholder='Qty' value='$stock->qty'>
						</div>
				</div>				
				</div>
			</div>
			<div class='form-group row'>
				...
				<div class='col-md-4'>
						...
						<div class='col-md-7'>
						<input type='number' class='form-control' id='total_qty'  required name='total_qty' placeholder='Qty' onkeyup='HitungJumlahMinus()'>
						</div>
				</div>
		...
		foreach ($gudangasal as $gudangasal) {
			echo "<option value='$gudangasal->id_gudang'>$gudangasal->nama_gudang</option>";
		};
		echo "</select>
								</div>
				</div>
	...
	public function SavePlus()
	{
		...
		$carigudang = $this->db->query("SELECT * FROM ms_gudang WHERE id_gudang ='$gudang' ")->result();
		...
		if ($adjustment == 'PLUS') {
			...
			$this->kartu_stok_in($material, $qtydo, $notr, $surat, $gudang);
			...
			$this->kartu_stok_out($material, $qtydo, $notr, $surat, $gudang);
			...
			$notr			= $code;
			$surat			= $code;
			$gudang			= $id_gudang_baru;
			$gudanglama		= $post['id_gudang'];


			if ($id_gudang_baru == 1) {
				$this->kartu_stok_in_penjualan($material, $qtyso, $notr, $surat, $gudang);
				if ($gudanglama == 1) {
					$this->kartu_stok_out_penjualan($material, $qtyso, $notr, $surat, $gudanglama);
				} else {
					$this->kartu_stok_out($material, $qtyso, $notr, $surat, $gudanglama);
				}
			} else {
				$this->kartu_stok_in($material, $qtyso, $notr, $surat, $gudang);
				if ($gudanglama == 1) {
					$this->kartu_stok_out_penjualan($material, $qtyso, $notr, $surat, $gudanglama);
				} else {
					$this->kartu_stok_out($material, $qtyso, $notr, $surat, $gudanglama);
	...
	public function kartu_stok_out($material, $qtyso, $notr, $surat, $gudang)
	{
		...
		$this->db->insert('kartu_stok_multigudang', $kartu);

		$this->db->query("UPDATE stock_material_multigudang SET qty=qty-$qtyso, qty_free=$free  WHERE id_category3='$material' AND id_gudang='$gudang'");
	}

	public function kartu_stok_out_penjualan($material, $qtyso, $notr, $surat, $gudang)
	{

		$mat = $this->db->query("SELECT * FROM stock_material WHERE id_category3='$material' AND id_gudang='$gudang'")->row();
		...
		$this->db->query("UPDATE stock_material SET qty=qty-$qtyso, qty_free=$free  WHERE id_category3='$material' AND id_gudang='$gudang'");
	}



	public function kartu_stok_in($material, $qtyso, $notr, $surat, $gudang)
	{
	...
	public function kartu_stok_in_penjualan($material, $qtyso, $notr, $surat, $gudang)
	{
	...
	public function konsinyasi()
	{
		...
		$gudang = $this->db->query("SELECT * FROM ms_gudang where id_gudang NOT IN('1','2','3','4')")->result();
		$data = [
			'gudang' => $gudang,
		];
		$this->template->set('results', $data);
		$this->template->title('Adjustment Stock Konsinyasi');
		$this->template->render('konsinyasi');
	}

	public function index_konsinyasi()
	{
		...
		$this->template->page_icon('fa fa-users');
		$deleted = '0';
		$stok		 = $this->Inventory_4_model->get_data_konsinyasi($gudang);
		$history	= $this->db->query("SELECT * FROM adjustment_stock ")->result();
		$data = [
			'stok' => $stok,
			'history' => $history,
		...
		$this->template->set('results', $data);
		$this->template->title('Adjustment Stock Konsinyasi');
		$this->template->render('index_konsinyasi');
	}
	...
	public function mutasi()
	{
		...
		$this->template->set('results', $data);
		$this->template->title('Mutasi Stock');
	...
	public function index_mutasi()
	{
		...
		$deleted = '0';
		if ($gudang == 1) {
			$stok		 = $this->Inventory_4_model->get_data_mutasi1($gudang);
		} else {
			$stok		 = $this->Inventory_4_model->get_data_mutasi2($gudang);
		}
		...
		$this->template->set('results', $data);
		$this->template->title('Mutasi Stock');
		$this->template->render('index_mutasi');
	}

	public function MutasiNow($id)
	{
		$id_category3 = $this->uri->segment(3);
		$id_stock = $this->uri->segment(4);
		$id_gudang = $this->uri->segment(5);
		...
		$this->template->set('results', $data);
		$this->template->title('Add Stock');
		$this->template->render('CreateMutasi');
	}
}
