				...
				<table id="exampleaa" class="table table-bordered table-striped">
					...
					<tbody>
						<?php if (empty($results)) {
							...
							$numb = 0;
							foreach ($results['history'] as $historia) {
								$numb++; ?>
								<tr>
									<td><?= $numb; ?></td>
									...
									<td><?= $historia->note ?></td>
									<td><?= $historia->total_jumlah_berat ?></td>
				...
				<table id="examplebb" class="table table-bordered table-striped">
					...
					<tbody>
						<?php if (empty($results)) {
							...
							$numb = 0;
							foreach ($results['stok'] as $record) {
								$numb++; ?>
								<tr>
									<td><?= $numb; ?></td>
									...
									<td>
										<?php
										$stock	= $this->db->query("SELECT SUM(qty) as totalweight FROM stock_material WHERE id_category3 = '$record->id_category3' ")->result();
										?>
										<?= number_format($stock[0]->totalweight, 2); ?> Kg
									</td>
									<td style="padding-left:20px">
										<?php if ($ENABLE_VIEW) : ?>
											<a class="btn btn-primary btn-sm edit" href="javascript:void(0)" title="Add Adjustment" data-id_inventory3="<?= $record->id_category3 ?>"><i class="fa fa-edit"></i></a>
										<?php endif; ?>
									</td>
...
<script type="text/javascript">
	$(document).on('click', '.edit', function(e) {
		...
		$.ajax({
			type: 'POST',
			url: siteurl + 'adjustmentstockmulti/AdjustNow/' + id,
