...
class Master_profit_material extends Admin_Controller
{
	//Permission
	...
	public function __construct()
	{
		...
		$this->load->model(array(
			'Master_profit_material/Lme_model',
	...
	public function index()
	{
		...
		$comp = $this->Lme_model->get_data('ms_profit_material');
		$data = [
			'history' => $history,
			'comp' => $comp
		];
		$this->template->set('results', $data);
		$this->template->title('Master Profit');
	...
	{
		$this->auth->restrict($this->viewPermission);
		$harga = $this->Lme_model->get_data('ms_profit_material');
		$tanggal = date('Y-m-d H:i:s');
		$usr 	= $this->auth->user_id();
		$inv = $this->Lme_model->get_data('ms_inventory_category1', 'deleted', '0');
		$nama_user = $this->Lme_model->get_data('users', 'id_user', $usr);
		$data = [
			'inv'	=> $inv,
			'harga' => $harga,
		...
		$this->template->set('results', $data);
		$this->template->render('edit_profit');
	}
