...
class Inventory_4_model extends BF_Model
{
    ...
    function level_3($type,$id_inventory2)
    {
		$search = "deleted='0' and id_type='$type' and id_category1='$id_inventory2'";
        $this->db->where($search);
    ...
    // {
		// $search = "deleted='0' and id_category2='$id_inventory3'";
        // $this->db->where($search);
	...
	public function dimensy($id){
		...
		$this->db->where($search);
        $this->db->order_by('name_suplier', 'ASC');
		$query = $this->db->get();		
		return $query->result();
	}
	...
	public function kompos($id){
		...
		$this->db->where($search);
        $this->db->order_by('name_compotition', 'ASC');
		$query = $this->db->get();		
		return $query->result();
	}
	...
	public function getview($id){
		...
		$this->db->from('ms_inventory_category3 a');
		$this->db->join('ms_bentuk b','b.id_bentuk=a.id_bentuk','left');
