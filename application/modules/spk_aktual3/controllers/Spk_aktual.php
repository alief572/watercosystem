...
class Spk_aktual extends Admin_Controller
{
	...
	public function SaveLHP()
	{
		...
		$getID_Lot = $this->db->select('id_stock')->get_where('dt_tr_spk_produksi', array('id_tr_spk_produksi' => $code2))->result();
		$ID_LOT = (!empty($getID_Lot[0]->id_stock)) ? $getID_Lot[0]->id_stock : NULL;

		$ArrSPKPro = [];

		if ($ID_LOT != NULL) {
			$getID_LotArray = $this->db->select('id_spkproduksi, id_tr_spk_produksi')->get_where('dt_tr_spk_produksi', array('id_stock' => $ID_LOT))->result_array();
			...
			foreach ($getID_LotArray as $valY => $valYx) {
				...
				foreach ($Detail as $val2 => $valx2) {
					...
					$ArrSPKPro1[] = $valYx['id_tr_spk_produksi'];
				}
			}
		}
		...
		if (!empty($ArrSPKPro2)) {
			...
			$this->db->where_in('id_tr_spk_produksi', $ArrSPKPro3);
	...
	public function SaveEditHeader()
	{
		...
		$data = [
			...
			'weight_actualpl'		=> $post['weight_actualpl'],
			'kg_sisa'				=> $post['kg_sisa'],
			'kg_sisa_actual'		=> $post['kg_sisa_actual'],
			'ng_packinglist1'		=> $post['ng_packinglist1'],
			'actualweightproses'	=> $post['actualweightproses'],
			'lebar_ng_in'			=> $ngin_lebar1[0],
			'berat_ng_in'			=> $ngin_berat1[0],
			'lebar_ng_ek'				=> $ngek_lebar1[0],
			'berat_ng_ek'				=> $ngek_berat1[0],

			'widthproduksi'		    => $post['widthproduksi'],
			'weightproduksi'		=> $post['weightproduksi'],
			'widthmothercoil'		=> $post['widthmothercoil'],
			'weightproses'		    => $post['weightproses'],
			'widthsisa'		    => $post['widthsisa'],
			'weightsisa'		=> $post['weightsisa'],
			'bpackinglist'		=> $post['bpackinglist'],
			'baktualmother'		=> $post['baktualmother'],
			'ng_packinglist'		=> $post['ng_packinglist'],
			'baktualweight_proses'		=> $post['baktualweight_proses'],
			'sisa_aktualweight_proses'		=> $post['sisa_aktualweight_proses'],
			'lfinishgood'		    => $post['lfinishgood'],
			'bfinishgood'		    => $post['bfinishgood'],
			'tweightslitting'		=> $post['tweightslitting'],
			'aktual'		        => $post['aktual']
		];
		...
		$this->db->where('id_tr_spk_produksi', $id_spk_aktual)->update("dt_tr_spk_produksi", $ubahstatus);
