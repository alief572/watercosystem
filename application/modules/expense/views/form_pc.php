...
if(!isset($data->departement)){
		...
		// if(!empty($data_employee)) {
			...
			// $data_head = $this->db->get_where('ms_petty_cash', ['approval' => $data_employee->username])->row();
			// $app=$data_head->approval;
		// }
}
$budgets=0;
?>
		...
		<div class="box box-primary">
			<div class="box-body">
				<div class="form-group ">
					...
					<div class="col-sm-4 col-md-4">
						<input type="date" class="form-control tanggal" id="tgl_doc" name="tgl_doc" value="<?php echo (isset($data->tgl_doc) ? $data->tgl_doc: date("Y-m-d")); ?>" placeholder="Tanggal Dokumen" required>
					</div>
				</div>
				<div class="form-group ">
				...
				<div class="form-group ">
					...
					<div class="col-md-2">
						<input type="text" class="form-control" id="bank_id" name="bank_id"  placeholder="Bank">
					</div>
					<label class="col-md-2 control-label">Nomor Rekening</label>
					<div class="col-md-2">
						<input type="text" class="form-control" id="accnumber" name="accnumber" placeholder="Nomor Rekening">
					</div>
					<label class="col-md-2 control-label">Nama Rekening</label>
					<div class="col-md-3">
						<input type="text" class="form-control" id="accname" name="accname"  placeholder="Nama Pemilik Rekening">
					</div>
				</div>
			</div>
			...
			<table class="table table-bordered table-striped" width="100%">
				...
				if(!empty($data_detail)){
					...
					<tr id='tr1_<?=$idd?>' class='delAll <?=($record->id_kasbon!=''?'kasbonrow':'')?>'>
						...
						?>
						<input type="date" class="form-control tanggal input-sm" name="tanggal[]" id="tanggal<?=$idd;?>" value="<?=$record->tanggal;?>" <?=$tekskasbon?> ></td>
...
//			data_save();
			...
			success: function(data){
				...
				for(i=0; i<data.length; i++){
					var Rows	 = 	"<tr id='tr1_"+nomor+"' class='delAll kasbonrow'>";
						...
						Rows	+=			"<input type='date' class='form-control tanggal input-sm' name='tanggal[]' id='tanggal_"+nomor+"' tabindex='-1' readonly value='"+data[i].tgl_doc+"' />";
	...
	function add_detail(){
		var Rows	 = 	"<tr id='tr1_"+nomor+"' class='delAll'>";
			...
			Rows	+=			"<input type='date' class='form-control tanggal input-sm' placeholder='Tanggal' name='tanggal[]' id='tanggal_"+nomor+"' />";
			Rows	+= 		"</td>";
			...
			Rows	+= 		"</td>";
			Rows	+= 		"<td data-header='Bon Bukti'>";
			Rows	+=			"<input type='file'  name='doc_file_"+nomor+"' id='doc_file_"+nomor+"' style='width: 90px' />";
			Rows	+= 		"</td>";
