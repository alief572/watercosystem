...
class Delivery_order extends Admin_Controller
{
	...
	function GetPenawaran()
	{
		...
		foreach ($dt as $dt) {
			...
			if (!empty($lot)) {
				...
				<td><input type='text' class='form-control input-sm text-right autoNumeric' value='$lot->qty' placeholder='Qty' id='dp_qty_mat_$loop$numb' required name='dp[$loop$numb][qty_mat]'></td>
				<td><input type='text' class='form-control input-sm text-right autoNumeric' placeholder='Weight' id='dp_weight_mat_$loop$numb' required name='dp[$loop$numb][weight_mat]' value='$lot->weight'></td>
	...
	function AddGetPenawaran()
	{
		...
		$lot			= $this->db->query("SELECT * FROM stock_material WHERE id_gudang = '3' AND id_category3='$id_category3' AND width = $width AND no_surat!=''")->result();
		...
		foreach ($lot as $lot) {
			...
			echo "<option value='$lot->lotno'>$lotslit</option>";
		}
		echo "</select>
	...
	function getStockLot()
	{
		...
		$get_stock 	= $this->db->get_where('stock_material', array('lotno' => $lot))->result();
	...
	public function SaveNewHeaderEdit()
	{
		...
		if (!empty($_POST['dp2'])) {
			foreach ($_POST['dp2'] as $val => $dt2) {
				...
				$ArrDetail[$val . $vaX]['id_delivery_order']		= $id_delivery;
				$ArrDetail[$val . $vaX]['id_dt_delivery_order']	= $id_delivery . '-' . $numb2;
				$ArrDetail2[$val . $vaX]['id_material']		    = $dt2['id_material'];
				$ArrDetail2[$val . $vaX]['nm_material']		    = $dt2['material'];
				$ArrDetail2[$val . $vaX]['no_alloy']		        = $dt2['no_alloy'];
				$ArrDetail2[$val . $vaX]['thickness']		        = $dt2['thickness'];
