...
class Incoming extends Admin_Controller
{
	...
	public function index()
	{
		...
		$data = $this->db->query("SELECT a.*, b.name_suplier as name_suplier FROM tr_incoming as a INNER JOIN master_supplier as b on a.id_suplier=b.id_suplier ORDER BY a.created_date DESC")->result();
	...
	public function TambahData()
	{
		...
		<td				hidden	><input  type='date' 		value='" . $tanggal . "'				class='form-control input-sm' id='dt_tanggal_" . $id . "_" . $no . "' 			required name='dt[" . $id . "][detail][" . $no . "][tanggal]' ></td>
		...
		<td				        ><input  type='text' 											class='form-control input-sm text-right' id='dt_qtyrecive_" . $id . "_" . $no . "' 		required name='dt[" . $id . "][detail][" . $no . "][qtyrecive]' 	></td>
		<td				        ><input  type='text' 		value='" . number_format($material->hargasatuan) . "'									class='form-control input-sm text-right' id='dt_hargasatuan_" . $id . "_" . $no . "' 		required name='dt[" . $id . "][detail][" . $no . "][hargasatuan]' 	></td>
	...
	function FormPo()
	{
		...
		$listpo = $this->db->query("SELECT 
									...
									WHERE 
										a.id_suplier='" . $id_suplier . "' AND b.sts_incoming IS NULL AND a.status = '2' GROUP BY a.no_po ")->result();
	...
	public function SaveNew()
	{
		...
		foreach ($Detail as $val => $valx) {
			...
			foreach ($valx['detail'] as $val2 => $valx2) {
				...
				$totalnilai  = $hargasatuan * $qtydo;

				$totalhutangkurs += $totalnilai;
				...
				$this->kartu_stok_in($material, $qtydo, $notr, $totalnilai, $nomor_po, $nilai_kurs, $noincoming);
			}
			...
			$ArrHeader	 = [
				...
				'matauang'		    => $kurs,
				'kurs'		        => $nilai_kurs,
				'hutang_kurs'		=> $totalhutangkurs,
				'hutang_idr'		=> $totalhutangkurs * $nilai_kurs,
				'no_po'				=> $nomor_po,
			];
	...
	function print_incoming_fix()
	{
        ...
        <hr>
       	<div id="footer">
        <table>
            <tr><td>PT Waterco Indonesia - Printed By ' . ucwords($session['username']) . ' On ' . $tglprint . ' </td></tr>
        </table>
        </div>
        ');
	...
	public function kartu_stok_in($material, $qtyso, $notr, $totalnilaido, $nomor_po, $kurs, $noincoming)
	{
		...
		$hutang_usaha =  $totalnilaido;
		$hutang_usaha_idr = $totalnilaido * $kurs;
