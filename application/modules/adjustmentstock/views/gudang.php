...
<div class="box">
	...
	<div class="box-body">
		<table id="example1" class="table table-bordered table-striped">
			...
			<tbody>
				<?php if (empty($results['gudang'])) {
				} else {
					foreach ($results['jumlah'] as $jumlah) {
					}
					$numb = 0;
					foreach ($results['gudang'] as $record) {
						$numb++; ?>

						<tr>
							<td><?= $numb; ?></td>
							<td><?= $record->lotno ?></td>
							<td><?= $record->nama_material ?>-<?= $record->thickness ?></td>
							<td><?= $record->width ?></td>
							<td><?= $record->length ?></td>
							<td><?= $record->qty ?> <?php if (empty($record->length)) {
														echo "Coil";
													} else {
														echo "Sheet";
													} ?></td>
